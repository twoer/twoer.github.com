...
{   
	...
	$(document).delegate('.upload-multiple-file .add-file', 'click', function()
	{
		...
		var _count = _uploadFileSender.attr('data-upload-count');

		if(_validateFileCount(_count) !== true )
		{
			$.microTip(
		    {
		        message : _lang.format('upload-count-limit', _count)
		    }, this);
			return;
		}
	...
	function _validateFileCount(count)
	{
		var _licount = _uploadFileSender.find('li[data-file-id]').size();
		//var _hiddencount = _uploadFileSender.find('input:hidden').val().split(',').length;
